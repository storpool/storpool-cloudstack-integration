...
class TestMigrationFromUuidToGlobalIdVolumes(unittest.TestCase): 
    UUID = ""
    GLOBALID = ""
    ARGS= ""
    @classmethod
...
#snapshot on secondary with uuid to template bypassed
    def test_19_template_from_snapshot_on_secondary_uuid(self):
        self.helper.storpool_snapshot_uuid(self.snapshot_uuid_on_secondary)
        ...
        self.helper.create_vm_from_template(template, True)

        self._cleanup.append(template)


#snapshot bypassed with uuid to template without bypassing secondary - This wont work

#snapshot bypassed with uuid to template bypassed
    def test_20_template_from_bypassed_snapshot_uuid(self):
        self.helper.storpool_snapshot_uuid(self.snapshot_uuid_bypassed)

        self.helper.bypass_secondary(True)

        template = self.helper.create_template_from_snapshot_or_volume(services = self.services, snapshotid = self.snapshot_uuid_bypassed.id)

        self.helper.create_vm_from_template(template, True)

        self._cleanup.append(template)

#snapshot with global id bypassed to template from volume with uuid bypass
    def test_21_template_from_snapshot_glid_bypassed(self):
...
#snapshot with global id on secondary to template from volume with uuid bypass
    def test_22_template_from_snapshot_glid_on_secondary(self):
...
#snapshot with global id on secondary to template from volume with uuid on secondary
    def test_23_template_from_snapshot_glid_on_secondary(self):
    ...
    def test_24_volume_from_snapshot_secondary(self):
    ...
    def test_25_volume_from_snapshot_bypassed(self):
        '''Create volume from snapshot (with uuid) on secondary, which volume was deleted
            and snapshot does not exists on StorPool or snapshot_details DB table
        '''
