...
import com.cloud.agent.api.storage.StorPoolMigrateWithVolumesCommand;
import com.cloud.agent.api.storage.StorpoolBackupTemplateFromSnapshotCommand;
import com.cloud.agent.api.to.DataObjectType;
import com.cloud.agent.api.to.StorPoolVirtualMachineTO;
...
public class StorPoolDataMotionStrategy implements DataMotionStrategy {
    ...
    private static final String SUPPORTED_VERSION = "4.15.0.0";
    ...
    public StrategyPriority canHandle(Map<VolumeInfo, DataStore> volumeMap, Host srcHost, Host destHost) {
        final CloudStackVersion dbVersion = CloudStackVersion.parse(versionDao.getCurrentVersion());
        final CloudStackVersion currentVersion = CloudStackVersion.parse(SUPPORTED_VERSION);
    ...
    final StrategyPriority canHandleLiveMigrationOnStorPool(Map<VolumeInfo, DataStore> volumeMap,
            Host srcHost, Host destHost) {
        if (srcHost.getId() != destHost.getId() && isPrimaryStorageSupported(volumeMap)
                && isDestinationStorPoolPrimaryStorage(volumeMap)) {
            return StrategyPriority.HIGHEST;
        }
        return StrategyPriority.CANT_HANDLE;
    }

    protected boolean isPrimaryStorageSupported(Map<VolumeInfo, DataStore> volumeMap) {
        if (MapUtils.isNotEmpty(volumeMap)) {
            for (VolumeInfo volumeInfo : volumeMap.keySet()) {
                StoragePoolVO storagePoolVO = _storagePool.findById(volumeInfo.getPoolId());
                return storagePoolVO != null
                        && (storagePoolVO.getStorageProviderName().equals(StorpoolUtil.SP_PROVIDER_NAME)
                                ...
                                || storagePoolVO.getPoolType() == StoragePoolType.Filesystem);
            }
        }
        return false;
    }
        ...
        try {
            if (srcHost.getHypervisorType() != HypervisorType.KVM) {
                throw new CloudRuntimeException(
                        "Invalid hypervisor type (only KVM supported for this operation at the time being)");
            }

            VMInstanceVO vmInstance = _vmDao.findById(vmTO.getId());
            StorPoolVirtualMachineTO spVmTO = new StorPoolVirtualMachineTO();
            try {
                BeanUtils.copyProperties(spVmTO, vmTO);
            } catch (IllegalAccessException iae) {
                throw new CloudRuntimeException("Cannot copy properties from VM transfer object", iae);
            } catch (InvocationTargetException ite) {
                throw new CloudRuntimeException("Cannot copy properties from VM transfer object", ite);
            }
            spVmTO.setState(vmInstance.getState());
            ...
            for (Map.Entry<VolumeInfo, DataStore> entry : volumeDataStoreMap.entrySet()) {
                ...
                conn = StorpoolUtil.getSpConnection(destDataStore.getUuid(), destDataStore.getId(), _storagePoolDetails, _storagePool);
                SpApiResponse resp = StorpoolUtil.volumeCreate(srcVolume.getUuid(), null, srcVolume.getSize(), vmTO.getUuid(), null, "volume", srcVolume.getMaxIops(), conn);
            ...
            PrepareForMigrationCommand pfmc = new PrepareForMigrationCommand(spVmTO);
            ...
            StorPoolMigrateWithVolumesCommand migrateCommand = new StorPoolMigrateWithVolumesCommand(vmTO.getName(), destHost.getPrivateIpAddress(),
                    isWindows, vmTO, true);
            ...
            boolean kvmAutoConvergence = Boolean.parseBoolean(_configDao.getValue("kvm.auto.convergence"));
            ...
            errMsg = String.format(
                    "Copy volume(s) of VM [%s] to storage(s) [%s] and VM to host [%s] failed in StorPoolDataMotionStrategy.copyAsync. Error message: [%s].",  vmTO.getId(), srcHost.getId(), destHost.getId(),
                    ex.getMessage());
            log.error(errMsg, ex);
