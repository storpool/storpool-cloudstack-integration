...
public class StorpoolVMSnapshotStrategy extends DefaultVMSnapshotStrategy {
  ...
  public VMSnapshot takeVMSnapshot(VMSnapshot vmSnapshot) {
       ...
       try {
            ...
            StorpoolCreateVMSnapshotCommand cmd = new StorpoolCreateVMSnapshotCommand(vmSnapshot.getUuid(), vmSnapshotVO.getUuid(),
                      userVm.getId(), target, volumeTOs, guestOS.getDisplayName());
  ...
  public boolean deleteVMSnapshot(VMSnapshot vmSnapshot) {
       ...
       try {
           ...
           GuestOSVO guestOS = guestOSDao.findById(userVm.getGuestOSId());
           StorpoolDeleteSnapshotVMCommand deleteSnapshotCommand = new StorpoolDeleteSnapshotVMCommand(vmInstanceName, vmSnapshotTO, volumeTOs, guestOS.getDisplayName());

           Answer answer = agentMgr.send(hostId, deleteSnapshotCommand);

           log.info("Delete vm nsapshot answer" + answer);
           if (answer != null && answer.getResult()) {
               ...
               log.error("Delete vm snapshot " + vmSnapshot.getName() + " of vm " + userVm.getInstanceName() + " failed due to " + errMsg);
