...
import com.cloud.vm.snapshot.VmWorkCreateVMSnapshot;
import com.cloud.vm.snapshot.VmWorkDeleteAllVMSnapshots;
import com.cloud.vm.snapshot.VmWorkDeleteVMSnapshot;
import com.cloud.vm.snapshot.VmWorkRevertToVMSnapshot;
import com.cloud.vm.snapshot.dao.VMSnapshotDao;
import com.cloud.vm.snapshot.dao.VMSnapshotDetailsDao;

@Component
@EnableAspectJAutoProxy(proxyTargetClass = true)
public class StorpoolVMSnapshotManagerImpl extends MutualExclusiveIdsManagerBase
implements VMSnapshotManager, VMSnapshotService, VmWorkJobHandler, Configurable{
     ...
     @Inject
     protected SnapshotDetailsDao _snapshotDetailsDao;
     @Inject
     public  PrimaryDataStoreDao storagePool;
     ...
     static final ConfigKey<Long> VmJobCheckInterval = new ConfigKey<Long>("Advanced", Long.class,
               "vm.job.check.interval", "3000", "Interval in milliseconds to check if the job is complete", false);
     ...
     public Pair<List<? extends VMSnapshot>, Integer> listVMSnapshots(ListVMSnapshotCmd cmd) {
          ...
          Filter searchFilter = new Filter(VMSnapshotVO.class, "created", false, cmd.getStartIndex(),
                    cmd.getPageSizeVal());
          ...
          if (state == null) {
               VMSnapshot.State[] status = { VMSnapshot.State.Ready, VMSnapshot.State.Creating,
                         VMSnapshot.State.Allocated, VMSnapshot.State.Error, VMSnapshot.State.Expunging,
      ...
      * Persistence and support for custom service offerings are done on the same
      * transaction
      ...
      */
     protected VMSnapshot createAndPersistVMSnapshot(UserVmVO userVmVo, String vsDescription, String vmSnapshotName,
               String vsDisplayName, VMSnapshot.Type vmSnapshotType) {
          final Long vmId = userVmVo.getId();
          final Long serviceOfferingId = userVmVo.getServiceOfferingId();
          final VMSnapshotVO vmSnapshotVo = new VMSnapshotVO(userVmVo.getAccountId(), userVmVo.getDomainId(), vmId,
                    vsDescription, vmSnapshotName, vsDisplayName, serviceOfferingId, vmSnapshotType, null);
          return Transaction.execute(new TransactionCallbackWithException<VMSnapshot, CloudRuntimeException>() {
               ...
               public VMSnapshot doInTransaction(TransactionStatus status) {
                    ...
                    s_logger.debug("Snapshot strategy"
                              + storageStrategyFactory.getVmSnapshotStrategy(vmSnapshot));
                    if (vmSnapshot == null) {
     ...
     /**
      * Add entries on vm_snapshot_details if service offering is dynamic. This will
      * allow setting details when revert to vm snapshot
      * @param vmId              vm id
          ...
          if (serviceOfferingVO.isDynamic()) {
               ...
               for (UserVmDetailVO detail : vmDetails) {
                    if (detail.getName().equalsIgnoreCase(VmDetailConstants.CPU_NUMBER)
                         ...
                         vmSnapshotDetails.add(new VMSnapshotDetailsVO(vmSnapshotId, detail.getName(),
                                   detail.getValue(), detail.isDisplay()));
                    }
               }
     ...
     public VMSnapshot createVMSnapshot(Long vmId, Long vmSnapshotId, Boolean quiescevm) {
          ...
          if (userVm == null) {
               throw new InvalidParameterValueException(
                         "Create vm to snapshot failed due to vm: " + vmId + " is not found");
          }
     ...
     private VMSnapshot orchestrateCreateVMSnapshot(Long vmId, Long vmSnapshotId, Boolean quiescevm) {
          ...
          if (!rootVolume.getState().equals(Volume.State.Ready)) {
               throw new CloudRuntimeException("Create vm to snapshot failed due to vm: " + vmId + " has root disk in "
                         + rootVolume.getState() + " state");
          }
     ...
     public boolean hasActiveVMSnapshotTasks(Long vmId) {
          List<VMSnapshotVO> activeVMSnapshots = _vmSnapshotDao.listByInstanceId(vmId, VMSnapshot.State.Creating,
                    VMSnapshot.State.Expunging, VMSnapshot.State.Reverting, VMSnapshot.State.Allocated);
     ...
     public boolean deleteVMSnapshot(Long vmSnapshotId) {
          s_logger.debug("EVENT_VM_SNAPSHOT_DELETE");;
          Account caller = getCaller();
          ...
          // check VM snapshot states, only allow to delete vm snapshots in created and
          // error state
          if (VMSnapshot.State.Ready != vmSnapshot.getState() && VMSnapshot.State.Expunging != vmSnapshot.getState()
                    && VMSnapshot.State.Error != vmSnapshot.getState()) {
               throw new InvalidParameterValueException("Can't delete the vm snapshotshot " + vmSnapshotId
                         + " due to it is not in Created or Error, or Expunging State");
          }

          // check if there are other active VM snapshot tasks
          if (hasActiveVMSnapshotTasks(vmSnapshot.getVmId())) {
               List<VMSnapshotVO> expungingSnapshots = _vmSnapshotDao.listByInstanceId(vmSnapshot.getVmId(),
                         VMSnapshot.State.Expunging);
               if (expungingSnapshots.size() > 0 && expungingSnapshots.get(0).getId() == vmSnapshot.getId())
                    s_logger.debug("Target VM snapshot already in expunging state, go on deleting it: "
                              + vmSnapshot.getDisplayName());
               else
                    throw new InvalidParameterValueException(
                              "There is other active vm snapshot tasks on the instance, please try again later");
          }
     ...
     private boolean orchestrateDeleteVMSnapshot(Long vmSnapshotId) {
          ...
          List<VMSnapshot.State> validStates = Arrays.asList(VMSnapshot.State.Ready, VMSnapshot.State.Expunging,
                    VMSnapshot.State.Error, VMSnapshot.State.Allocated);
          // check VM snapshot states, only allow to delete vm snapshots in ready,
          // expunging, allocated and error state
          if (!validStates.contains(vmSnapshot.getState())) {
               throw new InvalidParameterValueException("Can't delete the vm snapshot " + vmSnapshotId
                         + " due to it is not in " + validStates.toString() + "States");
          }
     ...
     public UserVm revertToSnapshot(Long vmSnapshotId)
          ...
          if (userVm == null) {
               throw new InvalidParameterValueException(
                         "Revert vm to snapshot: " + vmSnapshotId + " failed due to vm: " + vmId + " is not found");
          }
          ...
          if (userVm.getState() == VirtualMachine.State.Running && vmSnapshotVo.getType() == VMSnapshot.Type.Disk
               ...
               /*
                * if (!getStorageProviderName(vmId)) { throw new
                ...
                * + " snapshot"); }
                */
               s_logger.debug("Revert vm snapshot Storpool");
          }
     ...
     /**
      * If snapshot was taken with a different service offering than actual used in
      * vm, should change it back to it
     ...
     /**
      * Update service offering on {@link userVm} to the one specified in
      * {@link vmSnapshotVo}
     ...
     protected void changeUserVmServiceOffering(UserVm userVm, VMSnapshotVO vmSnapshotVo) {
          ...
          if (!result) {
               throw new CloudRuntimeException(
                         "VM Snapshot reverting failed due to vm service offering couldn't be changed to the one used when snapshot was taken");
          }
          s_logger.debug("Successfully changed service offering to " + vmSnapshotVo.getServiceOfferingId() + " for vm "
                    + userVm.getId());
     }

     /**
      * Upgrade virtual machine {@linkplain vmId} to new service offering
      * {@linkplain serviceOfferingId}
      * @param vmId              vm id
      * @param serviceOfferingId service offering id
     ...
     private UserVm orchestrateRevertToVMSnapshot(Long vmSnapshotId)
               throws InsufficientCapacityException, ResourceUnavailableException, ConcurrentOperationException {
          ...
          // start or stop VM first, if revert from stopped state to running state, or
          ...
          if (userVm.getState() == VirtualMachine.State.Stopped
               ...
               try {
                    ...
                    s_logger.error("Start VM " + userVm.getInstanceName() + " before reverting failed due to "
                              + e.getMessage());
               ...
               if (userVm.getState() == VirtualMachine.State.Running
                    ...
                    try {
                         ...
                         s_logger.error("Stop VM " + userVm.getInstanceName() + " before reverting failed due to "
                                   + e.getMessage());
                         throw new CloudRuntimeException(e.getMessage());
                    }
               }
     ...
     /**
      * Update or add user vm details from vm snapshot for vms with custom service
      * offerings
      *
      * @param userVm       user vm
      * @param vmSnapshotVo vm snapshot
     ...
     protected void revertUserVmDetailsFromVmSnapshot(UserVmVO userVm, VMSnapshotVO vmSnapshotVo) {
          ...
          if (serviceOfferingVO.isDynamic()) {
               ...
               for (VMSnapshotDetailsVO detail : vmSnapshotDetails) {
                    userVmDetails.add(new UserVmDetailVO(userVm.getId(), detail.getName(), detail.getValue(),
                              detail.isDisplay()));
               }
     ...
     public RestoreVMSnapshotCommand createRestoreCommand(UserVmVO userVm, List<VMSnapshotVO> vmSnapshotVOs) {
          ...
          for (VMSnapshotVO vmSnapshotVO : vmSnapshotVOs) {
               if (vmSnapshotVO.getType() == VMSnapshot.Type.DiskAndMemory) {
                    ...
                    VMSnapshotTO vmSnapshotTO = new VMSnapshotTO(snapshot.getId(), snapshot.getName(),
                              snapshot.getType(), snapshot.getCreated().getTime(), snapshot.getDescription(),
                              snapshot.getCurrent(), parent, quiescevm);
          ...
          RestoreVMSnapshotCommand restoreSnapshotCommand = new RestoreVMSnapshotCommand(vmInstanceName, null,
     ...
     public boolean syncVMSnapshot(VMInstanceVO vm, Long hostId) {
          try {
               ...
               List<VMSnapshotVO> vmSnapshotsInExpungingStates = _vmSnapshotDao.listByInstanceId(vm.getId(),
                         VMSnapshot.State.Expunging, VMSnapshot.State.Reverting, VMSnapshot.State.Creating);
     ...
     public Outcome<VMSnapshot> createVMSnapshotThroughJobQueue(final Long vmId, final Long vmSnapshotId,
          ...
          VmWorkCreateVMSnapshot workInfo = new VmWorkCreateVMSnapshot(callingUser.getId(), callingAccount.getId(),
                    vm.getId(), VMSnapshotManagerImpl.VM_WORK_JOB_HANDLER, vmSnapshotId, quiesceVm);
     ...
     public Outcome<VMSnapshot> deleteVMSnapshotThroughJobQueue(final Long vmId, final Long vmSnapshotId) {
          ...
          // save work context info (there are some duplications)
          VmWorkDeleteVMSnapshot workInfo = new VmWorkDeleteVMSnapshot(callingUser.getId(), callingAccount.getId(),
                    vm.getId(), VMSnapshotManagerImpl.VM_WORK_JOB_HANDLER, vmSnapshotId);
          workJob.setCmdInfo(VmWorkSerializer.serialize(workInfo));
     ...
     public Outcome<VMSnapshot> revertToVMSnapshotThroughJobQueue(final Long vmId, final Long vmSnapshotId) {
          ...
          // save work context info (there are some duplications)
          VmWorkRevertToVMSnapshot workInfo = new VmWorkRevertToVMSnapshot(callingUser.getId(), callingAccount.getId(),
                    vm.getId(), VMSnapshotManagerImpl.VM_WORK_JOB_HANDLER, vmSnapshotId);
          workJob.setCmdInfo(VmWorkSerializer.serialize(workInfo));
     ...
     public Outcome<VirtualMachine> deleteAllVMSnapshotsThroughJobQueue(final Long vmId, final VMSnapshot.Type type) {
          ...
          VmWorkDeleteAllVMSnapshots workInfo = new VmWorkDeleteAllVMSnapshots(callingUser.getId(),
                    callingAccount.getId(), vm.getId(), VMSnapshotManagerImpl.VM_WORK_JOB_HANDLER, type);
          workJob.setCmdInfo(VmWorkSerializer.serialize(workInfo));
